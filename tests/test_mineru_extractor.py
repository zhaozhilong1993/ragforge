        ...
        messages=[
            ...
            },
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": f"现在输入的图片，有可能是文档的目录索引，也有可能是正文章节，也有可能都不是，请先判断是不是文档的纯粹的目录索引，如果是，请提取图中的：{fields}，以输出目录中的各个章节所对应的页码;如果不是纯粹的目录索引，不要提取。请你以JSON格式输出，不要输出```json```代码段。"},
                ],
            },
        ],
    )
...
for r in result:
    ...
    if '目录' in r_d:
        ...
        for i in r_d['目录']:
            ...
            obj['章节'] = i['章节']
            obj['页码'] = i['页码']
            if obj['页码'] and obj['页码']>= current_page_index:
                dic_result.append(obj)
                current_page_index = obj['页码']
                page_end = index_id
            else:
                continue
print('章节页码结果{}, 从index_id {}开始是正文'.format(dic_result,page_end))
        ...
        messages=[
            ...
            },
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": f"提取图中的：{fields}，请你以JSON格式输出，不要输出```json```代码段。"},
                ],
            },
        ],
    )
...
print("花费时间 {}".format(task_time_cost))
