...
#读取文件
for name in ['tttttt','2.pdf','1.pdf','3.pdf','4.pdf','5.pdf','162.pdf','604.pdf','FPGA抗辐射加固技术.pdf']:
            ...
            messages=[
                ...
                },
                {
                    ...
                    "content": [
                        ...
                        {"type": "text", "text": f"现在输入的图片，有可能是文档的目录索引，也有可能是正文章节，也有可能都不是，请先判断是不是文档的纯粹的目录索引，如果是，请提取图中的：{fields}，以输出目录中的各个章节所对应的页码;如果不是纯粹的目录索引，不要提取。请你以JSON格式输出，以目录为Key，值是一个章节索引的列表，列表中是章节作为key，页码作为Key，两个Key组成，不要输出```json```代码段。"},
                    ],
                },
            ],
        )
