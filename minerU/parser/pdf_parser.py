...
class MinerUPdfParser:
    ...
    def ocr_function(self, pagenum, img, chars, ZM=3, device_id: int | None = None):
        start = timer()
        ...
        for c in Recognizer.sort_Y_firstly(
            ...
            if abs(ch - bh) / max(ch, bh) >= 0.7 and c["text"] != ' ':
                self.lefted_chars.append(c)
                continue
            print("bxs[ii] is {}".format(bxs[ii]["text"]))
        ...
        for b in bxs:
            if not b["text"]:
                ...
                print("bxs b text exists {}".format(b["text"]))
            del b["txt"]
            
        print('boxes_to_reg is {}'.format(boxes_to_reg))
        texts = self.ocr.recognize_batch([b["box_image"] for b in boxes_to_reg], device_id)
        for i in range(len(boxes_to_reg)):
            boxes_to_reg[i]["text"] = texts[i]
            print("ocr get boxes_to_reg[i] x0 {} x1 {} top {} bottom {}, result {}".format(boxes_to_reg[i]['x0'],boxes_to_reg[i]['x1'],boxes_to_reg[i]['top'],boxes_to_reg[i]['bottom'],texts[i]))
            del boxes_to_reg[i]["box_image"]
        logging.info(f"__ocr recognize {len(bxs)} boxes cost {timer() - start}s")
        for b in bxs:
            if b["text"]:
                print('ocr result {}'.format(b['text']))
    ...
    def __images__(self, fnm, zoomin=3, page_from=0,
        ...
        async def __img_ocr_launcher():
            def __ocr_preprocess():
                #如果是英文的，返回空；否则返回chars作为后续处理的参考
                chars = self.page_chars[i] if not self.is_english else []
                #非英文的情况下，每个字符的高度排序通过median获取中间的值，加入到求均值使用的mean_height
                #非英文的情况下，每个字符的宽度排序通股票median获取中间的值，加入到求均值使用的mean_width
    ...
    def __call__(self, fnm, need_image=True, zoomin=3, return_html=False):
        #先解析每页的OCR结果,识别到的每页面的box和box的文本，成为一个数组，放到self.boxes中。
        #也就是self.boxes是每页的所有的box和box的文本。
        self.__images__(fnm, zoomin)
        #再进行布局识别,会对于识别到的内容打布局标签，进行调整
        self._layouts_rec(zoomin)
        #针对布局识别器识别为表格的，用表格识别器进行识别
...
class MinerUPdf(MinerUPdfParser):
    ...
    #[[page, x1, x2, y1, y2]]
    def _line_tag_(self, page_idx,bx):
        return "@@{}\t{:.1f}\t{:.1f}\t{:.1f}\t{:.1f}##" \
            .format("-".join([str(page_idx)]),bx[0], bx[1], bx[2], bx[3])
